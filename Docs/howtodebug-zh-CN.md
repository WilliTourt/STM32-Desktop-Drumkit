...
- 注释掉`cpp_main.cpp`中while循环的整个for循环（或所有循环中的其他内容），取消注释200-215行的代码块。不需要管"unused variables"的警告。
...
- 还原代码到初始状态，然后取消注释`cpp_main.cpp`中182-190行的代码块。
...
还有，如果你在之后使用过程中感觉某个鼓垫的力度反馈不太好时，也可以回来重新标定一遍参数。
