...
// OLED参数
#define OLED_PAGE 4            // OLED页数  // CHANGE: 8 -> 4
...
void OLED_Init()
{
  ...
  OLED_SendCmd(0xC8); // 0xC8 & 0xC0 控制上下反转  // CHANGE: 0xC0 -> 0xC8
  ...
  OLED_SendCmd(0xDF);
  OLED_SendCmd(0xA1); // 0xA0 & 0xA1 控制左右反转  // CHANGE: 0xA0 -> 0xA1

  OLED_SendCmd(0xA6);
  OLED_SendCmd(0xA8);

  OLED_SendCmd(0x1F); // CHANGE: 0x3F -> 0x1F
  ...
  OLED_SendCmd(0xDA);
  OLED_SendCmd(0x02); // CHANGE: 0x12 -> 0x02
...
// 	static uint8_t sendBuffer[2] = { 0x40 };
// 	sendBuffer[1] = data;
// 	OLED_Send(sendBuffer, 2);
// }
...
//     OLED_SendCmd(0xB0 + y); // Set page address
//     OLED_SendCmd(0x00 | (x & 0x0F)); // Set lower column address
//     OLED_SendCmd(0x10 | ((x & 0xF0) >> 4)); // Set higher column address
// }

// /**
//  * @brief Print text on OLED display
//  * @param x Starting column position (0-127)
//  * @param y Starting row position (0-height-1)
...
//                     OLED_SendData(ascii_8x6[charIndex][i]);
//                 }
//                 x += 6;
//             } else if (size == 16) {
...
//                         OLED_SendData(ascii_16x8[charIndex][row*8 + i]);
//                     }
//                 }
//                 x += 8;
//             }
