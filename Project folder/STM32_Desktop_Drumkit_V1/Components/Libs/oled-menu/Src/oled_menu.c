...
// CHANGE: Commented out example menu items
// /**
//  * @brief 主菜单项。
//  * 
//  * 定义了主菜单的静态项。
...
Menutypedef *currentMenu = NULL; // CHANGE: Replaced example menu with NULL

// CHANGE: Commented out button list
/* 
 * @brief 按键列表。
 * 
 * 存储所有按键的状态信息。
...
{
  .topIndex = 0,    // 顶部索引
  .bottomIndex = (MENU_VISIBLE_ITEM_COUNT - 1)  // 底部索引
};

uint8_t scrollBarHeight = 0;

// CHANGE: Commented out key ID variable
/* 
 * @brief 当前按键 ID。
 * 
 ...
 */
// CHANGE: Commented out button callback function
...
{
  OLED_Init();
  //memset(KeyList, 0, sizeof(KeyList)); // CHANGE: Commented out button list initialization
  ...
  {
    int16_t visibleCount = MENU_VISIBLE_ITEM_COUNT;
    if (currentMenu->itemCount < visibleCount)
    {
      screenIndex.bottomIndex = currentMenu->itemCount - 1;
    }
    else
    {
      screenIndex.bottomIndex = visibleCount - 1;
    }
  	frameWidth.targetVal = currentMenu->items[currentMenu->currentItemIndex].len * 6 + 4;
  }
  HAL_Delay(10); // CHANGE: 20 -> 10
}
...
void OLEDUI_Update()
{
    // CHANGE: Commented out button update logic
	/* if (KeyList[keyID - 1].updateFlag) {
		...
		// ScrollBar_Update();    // 更新滚动条显示 */     // CHANGE: ScrollBar has some display issue
		// switchCtrlBar_Update();// 更新开关控件条显示 */ // CHANGE: Not used
  	// }
}
 ...
 */
// CHANGE: Commented out KeyLongPress function
/* void KeyLongPress()
{
 ...
 */
// CHANGE: Commented out KeyShortPress function
/* void KeyShortPress()
{
  if (controlSelectionFlag == 0)
  {
...
// CHANGE: Added new menu control functions
// External menu control implementations
void Menu_MoveRight() {
	if (controlSelectionFlag == 0) {
		...
		if (currentMenu->items[currentMenu->currentItemIndex].control->mode == SWITCH_CTRL) {
			int *data = currentMenu->items[currentMenu->currentItemIndex].control->data;
			*data = (*data == 0) ? 1 : 0;
		} else if (currentMenu->items[currentMenu->currentItemIndex].control->mode == SLIDER_CTRL) {
...
void Menu_Confirm() {
	if (controlSelectionFlag == 0) {
		if (currentMenu->items[currentMenu->currentItemIndex].subMenu != NULL) {
			currentMenu = currentMenu->items[currentMenu->currentItemIndex].subMenu;
			currentMenu->currentItemIndex = 0;
			menuSwitchFlag = 1;
		} else if (currentMenu->items[currentMenu->currentItemIndex].Function != NULL) {
			currentMenu->items[currentMenu->currentItemIndex].Function();
...
uint8_t Menu_GoBack() {
	if (currentMenu->parentMenu && controlSelectionFlag == 0) {
		currentMenu = currentMenu->parentMenu;
		menuSwitchFlag = 1;
		return 1;
	}
  ...
  else
{
  if (currentMenu->currentItemIndex > screenIndex.bottomIndex)
  {
    ...
    screenIndex.bottomIndex = currentMenu->currentItemIndex;
      screenIndex.topIndex = screenIndex.bottomIndex - (visibleCount - 1);
      if (screenIndex.topIndex < 0)
      {
        screenIndex.topIndex = 0;
      }
  }
  else if (currentMenu->currentItemIndex < screenIndex.topIndex)
  {
    ...
    screenIndex.topIndex = currentMenu->currentItemIndex;
      screenIndex.bottomIndex = screenIndex.topIndex + (visibleCount - 1);
      if (screenIndex.bottomIndex > currentMenu->itemCount - 1)
      {
        screenIndex.bottomIndex = currentMenu->itemCount - 1;
      }
    }
...
void ScrollBar_Update(void)
{
  ...
  const int trackHeight = OLED_SCREEN_HEIGHT - (trackMargin * 2);

  if (currentMenu->itemCount == 0)
  {
    scrollBarHeight = trackHeight;
    scrollBarY.targetVal = trackMargin;
    return;
  }
  ...
  {
    float rawHeight = ((float)trackHeight * visibleCount) / currentMenu->itemCount;
    scrollBarHeight = (uint8_t)(rawHeight + 0.5f);
    if (scrollBarHeight < 4)
    {
      scrollBarHeight = 4;
    }

    int moveRange = trackHeight - scrollBarHeight;
    float step = (currentMenu->itemCount > 1) ? ((float)moveRange / (currentMenu->itemCount - 1)) : 0.0f;
    scrollBarY.targetVal = trackMargin + (int)(step * currentMenu->currentItemIndex + 0.5f);
    int maxTarget = trackMargin + moveRange;
    if (scrollBarY.targetVal > maxTarget)
    {
      scrollBarY.targetVal = maxTarget;
    }
  }
}
...
void switchCtrlBar_Update(void)
{
    ...
    // CHANGE: Added outer if-else to avoid accessing uninitialized pointers
    if (currentMenu && 
        currentMenu->items && 
        currentMenu->currentItemIndex < currentMenu->itemCount &&
        currentMenu->items[currentMenu->currentItemIndex].control &&
        currentMenu->items[currentMenu->currentItemIndex].control->data)
    {
		if (*(currentMenu->items[currentMenu->currentItemIndex].control->data) == 0)
		{
    ...
    {
		// Default position if no valid control data
...
void OLEDUI_Move(void)
{
  ...
  // UI_SmoothTransition(&scrollBarY, &moveProcess_ScrollBar, 0.1);        // CHANGE: ScrollBar has some display issue
  // UI_SmoothTransition(&switchCtrlBar, &moveProcess_SwitchCtrlBar, 0.1); // CHANGE: Not used
}
...
void DrawMenuItems()
{
  ...
  int16_t showItemStartNum = (screenIndex.topIndex > 0) ? -1 : 0;
  int16_t showItemEndNum = visibleCount + ((screenIndex.bottomIndex < currentMenu->itemCount - 1) ? 1 : 0);
  ...
  DrawSelectionFrame();
  // DrawScrollBar(); // CHANGE: ScrollBar has some display issue
...
void DrawScrollBar()
{
  const int trackX = OLED_SCREEN_WIDTH - SCROLLBAR_WIDTH - SCROLLBAR_MARGIN;
  const int trackY = 2;
  const int trackHeight = OLED_SCREEN_HEIGHT - (trackY * 2);
  int visibleCount = MENU_VISIBLE_ITEM_COUNT;
  if (visibleCount <= 0)
  {
    visibleCount = 1;
  }

  OLED_DrawRectangle(trackX - 2, 0, 6, OLED_SCREEN_HEIGHT - 1, OLED_COLOR_NORMAL);
  ...
  int barHeight = scrollBarHeight ? scrollBarHeight : trackHeight;
  if (currentMenu->itemCount <= visibleCount)
  {
    barHeight = trackHeight;
  }

  int barTop = scrollBarY.val;
  if (barTop < trackY)
  {
    barTop = trackY;
  }
  int maxBarTop = trackY + trackHeight - barHeight;
  if (barTop > maxBarTop)
  {
    barTop = maxBarTop;
  }

  OLED_DrawFilledRectangle(trackX, barTop, SCROLLBAR_WIDTH, barHeight, OLED_COLOR_NORMAL);
}
...
{
	// CHANGE: When the current item is a switch control, toggle its value directly
	// For switch controls, directly toggle the value
	if (currentMenu->items[currentMenu->currentItemIndex].control != NULL && 
		currentMenu->items[currentMenu->currentItemIndex].control->mode == SWITCH_CTRL) {
		int *data = currentMenu->items[currentMenu->currentItemIndex].control->data;
		*data = !(*data);
	} else {
		// For other controls, enter selection mode
